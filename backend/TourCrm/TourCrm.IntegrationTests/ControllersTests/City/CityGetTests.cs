...
public class CityGetTests(WebApplicationFactory<Program> f) : IClassFixture<WebApplicationFactory<Program>>
{
    [Fact]
    public async Task GetAll_200()
    {
        ...
        svc.Setup(s => s.GetAllAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<CityDto> { new() { Id = 1, Name = "Almaty" } });
        ...
        var list = await resp.Content.ReadFromJsonAsync<List<CityDto>>();
    ...
    public async Task Get_ById_200()
    {
        ...
        svc.Setup(s => s.GetByIdAsync(2, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new CityDto { Id = 2, Name = "Astana" });

        var req = new HttpRequestMessage(HttpMethod.Get, "/api/City/2");
        req.Headers.Add("x-test-perms", "ViewDictionaries");
        var resp = await c.SendAsync(req);
    ...
    public async Task Get_ById_404()
    {
        ...
        var req = new HttpRequestMessage(HttpMethod.Get, "/api/City/999");
        req.Headers.Add("x-test-perms", "ViewDictionaries");
        var resp = await c.SendAsync(req);
