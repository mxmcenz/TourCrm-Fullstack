...
public class MealTypeUpdateDeleteTests(WebApplicationFactory<Program> f) : IClassFixture<WebApplicationFactory<Program>>
{
    [Fact]
    public async Task Update_204_WithPermission()
    {
        var (c, svc) = TestClient.CreateWithHeaderAuthAndMock<IMealTypeService>(f);
        svc.Setup(s => s.UpdateAsync(5, It.IsAny<UpdateMealTypeDto>(), It.IsAny<CancellationToken>()))
           .Returns(Task.CompletedTask);

        var req = new HttpRequestMessage(HttpMethod.Put, "/api/MealType/5")
        {
            Content = JsonContent.Create(new UpdateMealTypeDto { Name = "FB" })
        };
    ...
    public async Task Delete_204_WithPermission()
    {
        ...
        var req = new HttpRequestMessage(HttpMethod.Delete, "/api/MealType/7");
