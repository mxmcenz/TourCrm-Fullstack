...
public class LeadRequestTypesController(ILeadRequestTypeService service) : ControllerBase
{
    [HttpGet]
    ...
    public async Task<IActionResult> Create([FromBody] CreateLeadRequestTypeDto dto, CancellationToken ct)
        => StatusCode(StatusCodes.Status201Created, await service.CreateAsync(dto, ct));

    [HttpPut("{id:int}")]
    [HasPermission("EditDictionaries")]
    public async Task<IActionResult> Update(int id, [FromBody] UpdateLeadRequestTypeDto dto, CancellationToken ct)
