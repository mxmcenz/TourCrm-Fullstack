    ...
    <v-main>
      <div class="full-page-bg">
        <v-container class="d-flex flex-column align-center pt-10 max-w-1200">
          <h1 class="text-h4 font-weight-medium mb-6">Установить пароль</h1>
          ...
          <v-card class="pa-6 w-100" max-width="420" elevation="8" rounded="lg">
            <v-form ref="form" v-model="valid" autocomplete="off">
              ...
              <v-text-field
                  ...
                  bg-color="white"
                  color="primary"
                  :rules="[rules.required, passwordValidator]"
              />

              <v-btn class="pill btn-pear mt-5" variant="flat" :ripple="false" block
                     :disabled="!valid || loading" @click="submit">
...
import {rules, matchPassword as matchPasswordRule} from '@/shared/utils/rules'
...
const passwordValidator = matchPasswordRule(password)
...
async function submit() {
  ...
  loading.value = true
  try {
    const userId = Number(localStorage.getItem('registerUserId'))
    ...
    error.value = err.response?.data?.message || err.message || 'Ошибка установки пароля'
  } finally {
    loading.value = false
  }
}
...
:deep(.v-form) { animation: slideUp 0.6s ease-out; }
@keyframes slideUp {
...
:deep(.v-alert__border) { opacity: .6; }
